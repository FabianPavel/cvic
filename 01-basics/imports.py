...
# Výpočet datumu Velikonoční neděle (Easter) v následujících 5 letech
dnes = datetime.date.today()
for i in range(1, 6):
    datum_velikonoci = easter.easter(dnes.year + i)
    print(f" Velikonoční neděle ({dnes.year + i}): {datum_velikonoci.strftime('%d.%m.%Y')}")

# Nalezení nejbližšího roku, kdy bude Štědrý den (Christmas Eve) v neděli
def najdi_nejblizsi_stedry_den():
    dnes = datetime.date.today()
    rok = dnes.year
    while True:
        if calendar.weekday(rok, 12, 24) == 6:  # Neděle má index 6
            return rok
        rok += 1

nejblizsi_stedry_den = najdi_nejblizsi_stedry_den()
print(" Nejbližší rok, kdy bude Štědrý den v neděli:", nejblizsi_stedry_den)
